module verlet
    ...
    subroutine get_delta_ser(p1, p2, p3, delta, ser)
        ...
        do dim = 1, 3, 1
            ser(1, dim) = eudist_with_delta(p1, p2, delta, dim)
            ser(2, dim) = eudist_with_delta(p1, p3, delta, dim)
            ser(3, dim) = eudist_with_delta(p2, p2, delta, dim)
        end do
    end subroutine get_delta_ser

    real(KIND=wp) function eudist_with_delta(p1, p2, delta, dim) result(dist)
        use kinds, ONLY: wp => dp
        implicit none
        ! Two points
        real(KIND=wp), DIMENSION(3), intent(in) :: p1, p2
    ...
    subroutine compute_force(points, delta, forces)
        ...
        do dim = 1, 3, 1
            delta_d_(1, dim) = eudist_with_delta(points(1, :), points(2, :), delta, dim)
            delta_d_(2, dim) = eudist_with_delta(points(1, :), points(3, :), delta, dim)
            delta_d_(3, dim) = eudist_with_delta(points(2, :), points(3, :), delta, dim)
        end do
